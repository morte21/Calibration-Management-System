...
namespace Calibration_Management_System.Controllers
{
    public class GeneralFormController : Controller
    {
        ...
        {
            var allData = _context.GeneralForm_table.Where(x => x.fld_docNo != null);

            // Create headers
            var headers = string.Join(",", typeof(GeneralForm).GetProperties().Select(prop => prop.Name));
            ...
            {
                var rowData = string.Join(",", typeof(GeneralForm).GetProperties().Select(prop => prop.GetValue(dataRow, null)));
