...
namespace Calibration_Management_System.Controllers
{
    public class EquipmentMasterController : Controller
    {
        ...
        {
            var allData = _context.Equipment_table.Where(x => x.fld_passFail == "PASS" && x.fld_stat == "OK").ToList();
           

            // Create headers
            var headers = string.Join(",", typeof(EquipmentRegistration).GetProperties().Select(prop => prop.Name));
            ...
            {
                var rowData = string.Join(",", typeof(EquipmentRegistration).GetProperties().Select(prop => prop.GetValue(dataRow, null)));
